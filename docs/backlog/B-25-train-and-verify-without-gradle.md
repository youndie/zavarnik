...
раннер без зависимостей. Толстый jar (`fatJar`, со stdlib) едет ресурсом внутри плагина, классы раннера — прямо в jar
плагина; отдельно модуль не публикуется (первая попытка публиковать его как
`io.github.youndie:zavarnik-runner` упёрлась в 403 от reposilite — токен CI выписан на пути
плагина; одна координата для потребителя и без того лучше). Задача `zavarnikRunnerFiles` кладёт
jar и `zavarnik.properties` в `lib/` через содержимое дистрибутива, поэтому они есть и в
`installDist`, и в tar, и в zip. Оба AC закрыты, журналы —

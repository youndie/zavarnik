...
> `portability = true` закреплено (D6). **Прогнано 06.09.2026 на паре EPYC Genoa (AVX-512) →
> Core Ultra 7 255HX (AVX2), тот же билд JDK:** `SIGILL` не воспроизвёлся — нативные кэши hello
> world и образца работают на узком CPU (`experiments/cpu-portability/`). Загрузчик 25.0.4 набор
> инструкций не проверяет, проверка появится в JDK 27 (`verify_cpu_features` в mainline). Отчёты о
> падении — про CPU без AVX (Core2) и Ryzen 3500X на Windows. **Остаётся:** положительный контроль —
> машина, на которой падение есть; без неё результат говорит «на этой паре не нужно», а не
> «не нужно». Умолчание не меняется.

...
> **Отложено владельцем 07.09.2026.** Механизм и цена умолчания измерены, падение на паре
> Genoa → Core Ultra не воспроизведено; искать машину без AVX ради положительного контроля не
> будем. `portability = true` остаётся умолчанием. В работу не берётся.

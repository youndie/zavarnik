...
**Правка 07.09.2026 (issue youndie/zavarnik#2, измерение владельца на bochka):** VM-половина
гипотезы «обе стадии на jre» закрыта — `eclipse-temurin:25-jre` **и пишет, и читает** свой кэш:
тренировка стартовым скриптом дистрибутива с `-XX:AOTCacheOutput` и `docker stop -t 20`
(SIGTERM), запуск под `-XX:AOTMode=on` — «Opened AOT cache», контроль с несуществующим путём —
`exit=1`. Старт bochka по её же `started in N ms`, семь прогонов: медиана 1377 → 1225 мс (−11 %),
разброс 126 → 31 мс. Что не закрыто: Gradle на JRE — и не нужно закрывать, если тренировать в
JRE-стадии запуском установленного дистрибутива, а не `aotTrain`; тогда нужен способ выполнить
готовность, нагрузку, SIGTERM, манифест и проверки без Gradle и без `curl` (в образе Temurin его
нет) — [B-25](../backlog/B-25-train-and-verify-without-gradle.md). Проверено на образце тем же
днём: `-jdk` собирает `installDist`, `-jre` копирует и тренирует стартовым скриптом с
`timeout -s TERM 12` — кэш 26 МБ, образ 527 МБ вместо 647, под `-XX:AOTMode=on` стартует; но без
готовности и нагрузки из кэша приходят **7 из 13** классов `sample.*` и 325 из 591 Ktor — рецепт
...
`experiments/runner-jre/results/`. Следствие 5 в исходной форме («тренируй в стадии сборки на
том же `-jdk`») остаётся верным как рецепт для тех, кто не хочет раннера, но больше не
единственным.
...
**Прогрев JIT — что кэш греет, а что нет (07.09.2026).** Вопрос владельца: кэш ускоряет только
старт или и горячие запросы? По устройству: JEP 515 кладёт в кэш профили методов с тренировки,
и `workload { }` в `aotTrain` определяет, какие запросы окажутся «горячими» в этих профилях; но
скомпилированного кода в кэше JDK 25/26 нет (AOT-компиляция — черновик JEP 8335368), только
адаптеры и стабы. Измерено на `bench/`, `/business`, 32 соединения, 20 с нагрузки после
готовности, `-XX:+CITime` при выходе, три повтора (`experiments/jit-warmup/`):

| Вариант | C1: методов | C2: методов | C2: время, с |
|---|---|---|---|
| холодный (`-XX:AOTMode=off`) | 3834 / 3914 / 4268 | 1373 / 1354 / 1620 | 22,9 / 24,7 / 30,9 |
| с кэшем | 3513 / 3964 / 3818 | 1242 / 1656 / 1454 | 19,8 / 17,4 / 30,2 |

(Журнал `results/2026-09-07-…-citime.log`. Первый, несохранённый прогон того же дня дал
3776–3853 / 3343–3495 по C1 и 1309–1402 / 1151–1241 по C2 — те же величины.)

**Следствие.** С кэшем JIT после старта делает **ту же работу с точностью до шума**: в пяти
парах из шести компиляций на 8–12 % меньше, в одной — больше, потому что число скомпилированных
методов растёт вместе с тем, сколько запросов успело пройти за 20 с, а это на этой машине
гуляет. Профили из JEP 515 — экономия порядка десятой части, не «тёплый» JIT: код компилируется
заново. Кэш — про старт, линковку, архивированную кучу и
профили; «тёплый» сервис с первого запроса он не даёт, и `aotReport` этого не показывает — он
меряет только время до первого `200`. Кривая rps по двухсекундным окнам (`warmup-curve.sh`) на этой
машине утонула в шуме частоты (между повторами — четырёхкратный разброс), но первое окно у обоих
вариантов — 15–50 % от установившегося, и с кэшем оно не выше. Сделано: [B-24](../backlog/B-24-aot-report-warmup-curve.md) — `aotReport`
печатает окно JIT второй таблицей; на образце Ktor C2 1171 метод холодным против 1172 с кэшем.

**Память (09.09.2026, `experiments/memory/`).** RSS образца по `/proc/<pid>/status`, пять запусков на
вариант попеременно, Linux 25.0.4: в момент готовности 85,0 МиБ без кэша против 84,7 с ним, после
200 запросов 122,9 против 119,6 — разница меньше разброса любого ряда, `RssFile` те же ~22 МиБ.
Кэш в 31 МиБ в резидентной памяти не виден; его цена — диск (файл и слой образа) и тренировка
(вторая JVM сборки кэша, по JEP 514 — двойной heap).
...
**Правка 07.09.2026 (B-28, по эксперименту на konekt):** константу ставит уже `installDist`
(`doLast` плагина), а не только `aotTrain`. Причина: когда сервис не стартует без базы,
тренировка идёт раннером внутри контейнера образа, собранного из `installDist`, и то, что раннер
пинит у себя в контейнере, образ не видит — второй образ с кэшем слоем получил бы jar-ы со
временем копирования и отказ «timestamp has changed». Проверено на konekt обходом `touch` на
хосте; теперь обход не нужен.

**Правка 07.09.2026 (B-26, Jib):** для Jib-образа константа не нужна вовсе — раннер тренирует
внутри контейнера того самого образа, mtime не трогает, а кэш докладывается следующим Jib-билдом
как слой поверх тех же jar-ов с тем же `EPOCH_PLUS_SECOND`. «Jib без настроек» вернулось, но
другим путём: не совпадением констант, а тренировкой на месте. Раскладка `exploded` (умолчание Jib
и `ktor { docker { } }`) отказывается на конфигурации — каталоги на classpath, кэша нет (E4).
...
[про закрытую вторую фазу](https://kotlin.website/blog/user-code-share-of-a-ktor-service)
([research-optimizer](research-optimizer.md)); с них идёт отсчёт критерия остановки 4 из брифа
(четыре недели без внешней реакции) — до 05.10.2026.

**Первое применение на чужом сервисе (07.09.2026, konekt, B-27 → konekt B-123):** готовность
`docker start` → `/health` под лимитом в одно ядро 4380 → 2042 мс и первый запрос 510 → 240 мс по
...
`readdir` конкретной файловой системы, и он **разный у разных рантаймов**. Проверка 07.09 (W1–W4,
одна машина, одна ФС) была недостаточной; `aotVerify` бессилен, потому что проверяет там, где
тренировали. Плагин теперь отказывает на wildcard в `CLASSPATH`; konekt перечисляет jar-ы явно.

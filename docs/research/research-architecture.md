...
**Что проверено 06.09.2026 (B-09, частично):** флаг действительно убирает регион кода из кэша, а
цена на образце Ktor неизмерима — 173 против 174 мс медианы готовности (§1.6). Значит умолчание
`portability = true` ничего не стоит, и это уже не гипотеза. **Что не проверено:** само падение
`SIGILL` на чужом CPU — нужны две машины с разным набором инструкций, а Linux-машина без AVX-512
и мак на arm64 не составляют такой пары; пока это факт из чужих отчётов (§1.6).
...
умолчанию, убирает регион кода (проверено) и не стоит времени готовности (измерено). Открыт
только в части воспроизведения самого падения —
[B-09](../backlog/B-09-cpu-portability-adapter-caching.md) ждёт машины с AVX-512.

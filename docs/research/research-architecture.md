...
| `touch -t 197001010000.01` читает **местное** время: на Linux-машине дал `-3599` | тот же прогон; в плагине — `FileTime.fromMillis(1000)`, в харнессе — `TZ=UTC` |
...
не выражается. `distTar` может, если плагин выставит `preserveFileTimestamps = true` на задаче: tar
хранит секунды epoch, `1000 мс` → `1 с` → совпадает с тем, что проверяет JVM. Решение —
[B-07](../backlog/B-07-start-scripts-and-distribution-wiring.md): кэш в `distTar` с сохранением
времени, в `distZip` — не класть и предупредить; Docker `COPY` из `installDist` и Jib — как в D3.

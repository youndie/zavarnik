...
**По механике — красный для IR-слоя как продукта про производительность.** На самом
...
**Риск 1. Стенд написан мной и мал: 39 классов пользовательского кода.** Доля пользовательского
кода в настоящем сервисе может быть выше — но не в разы: `/business` уже собирает всё, что бриф
называл кандидатами, и даёт 10 %. Митигация: методика (`run.sh` + `attribute.py`) переносима на
любой сервис на Ktor; прежде чем строить IR-плагин, прогнать её на настоящем сервисе владельца
— это час, а не неделя. Открыт.

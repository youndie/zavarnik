...
| CPU-сэмплов при 50 rps под лимитом в одно ядро — 2817 за 120 с: сервис почти простаивает, и в «jvm» (61 % self) сидят потоки JIT (`PhaseChaitin::Split`, `IndexSetIterator`) | тот же журнал, top self frames; плотный профиль при 200 rps — ниже, если стенд выдержал |
...
сервисе ещё ниже, чем на стенде, потому что настоящий сервис — это база данных, сериализация и
фреймворк, а пользовательский код в нём — тонкая прослойка. D1 — без оговорок.

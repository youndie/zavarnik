...
трёх интерфейсов на Kotlin 2.4.10, четыре режима:

| режим | `DefaultImpls` | `access$cancel$jd` | затронут |
|---|---|---|---|
| без флага (умолчание 2.4.10) | есть | **есть** | **да** |
| `-jvm-default=enable` (= legacy `all-compatibility`) | есть | есть | да |
| `-jvm-default=no-compatibility` (= legacy `all`) | нет | нет | нет |
| `-jvm-default=disable` | есть | нет | нет |

В режиме совместимости компилятор выпускает и default-метод на интерфейсе, и легаси-класс
`DefaultImpls`; связать их `invokespecial`-ом из отдельного класса нельзя, поэтому на самом
интерфейсе синтезируется статический `access$cancel$jd`, и его тело — тот самый
`invokespecial` на текущий интерфейс. Проверено на настоящем jar-е:
`CompletableDeferred$DefaultImpls.cancel` делает `invokestatic … access$cancel$jd`, и
`DefaultImpls` в `kotlinx-coroutines-core-jvm-1.11.0.jar` присутствует (24 таких класса на 869).

**Написанное здесь раньше — «затронут любой интерфейс с `-Xjvm-default=all`» — было
перевёрнуто:** это ровно тот режим, где аксессора нет. Затронуто умолчание, отчего радиус и
покрывает весь стек coroutines/Ktor.

...
**Следствие 2 — уточнено 11.09.2026 прогоном на паре машин (B-35, `experiments/crac-cpu/`).**
Пара: AMD EPYC-Genoa (fornex, AVX-512) и Core Ultra 7 255HX (Linux-машина, только AVX2), один и тот
же образ по digest.

| Снимок снят на | Флаг | Восстановлен на | Итог |
|---|---|---|---|
| широком | нет | узком | **отказ**, код 1, сообщение называет маску |
| широком | `generic` | узком | **SIGSEGV**, код 139, ни строчки вывода |
| узком | `generic` | широком | **восстановился** |
| узком | нет | широком | **отказ**, код 1, то же сообщение |
| широком | `generic` | широком (своём) | восстановился — контроль: флаг снимок не ломает |

Без флага ограничение — **точное совпадение**, а не «набор снимка ⊆ набор машины»: отказывает и
направление узкий → широкий, где недостающих инструкций нет по определению. Движок печатает обе
битовые маски и слова «Image constraint 'cpu.features' (bitmap subset) does not match».

`-XX:CPUFeatures=generic` делает снимок переносимым **только вверх**. Снятый на узком CPU он
оживает на широком; снятый на широком — на узком **падает молча**, без сообщения JVM и без
...
ниже во **всех шести** парах — 5526 против 4089 запросов/с по медиане, ряды 4377–5711 и 2499–4956.
Величине не верить (ряды перекрываются, машина не изолирована), направлению — да.

**Две ловушки, обе встречены:** пустой каталог снимка тоже отвечает «incompatible or missing CPU
features», так что сорванная передача читается как вердикт о CPU; и `jcmd` не прикрепляется к JVM,
запущенной под неотображённым uid (`docker run --user "$(id -u)"`) — снимок не снимается, а пустой
каталог ведёт прямиком в первую ловушку.
...
### 1.9 Jib-режим: снимок переезжает, точка входа заменяется (11.09.2026, B-34)

Несущий вопрос упаковки — можно ли снять снимок в примонтированный каталог, а в образе держать его
по другому пути — проверен до того, как писался код (`experiments/crac-ktor/image-layer.sh`).

| Факт | Где проверено |
|---|---|
| Снимок, снятый в `/out/crac` внутри контейнера, восстанавливается из `/opt/app/zavarnik/crac` в образе, собранном как «тот же образ плюс слой»: «warp: Restore successful!», готовность **46 мс**, `/api/warm` отвечает 200 | `image-layer.sh`, прогон 11.09.2026 |
| **Но путь к файлу политик вморожен в снимок.** Первый прогон восстановился на уровне warp и упал в JVM: `OpenResourcePolicies$ConfigurationException: File /out/crac/zavarnik-crac-policies.yaml … does not exist` — JVM запоминает `jdk.crac.resource-policies` как строку и читает файл **снова** при восстановлении | тот же прогон, до исправления |
| Лечение: файл политик живёт **внутри образа**, рядом с `zavarnik.properties` (его пишет `RunnerFilesTask`, а раннер — если файла нет), и никогда в каталоге снимка | `Crac.policyFile()`, `RunnerFilesTask`, тест `CracTest` |
| Jib-путь целиком: `jibCracCheckpoint` → `jibCracVerify` → образ с точкой входа `["java","-XX:CRaCRestoreFrom=/app/zavarnik/crac"]`, поднимается за **174 мс** и отвечает 200 | `samples/ktor-jib/crac-check.sh`, прогон 11.09.2026; в CI |

**Следствие 1 — форма задач та же, кроме одного.** У AOT-кэша слой добавлял **флаг** к точке входа;
снимок её **заменяет**, потому что восстановление не несёт ни classpath, ни главного класса. Значит,
в одном образе живёт либо кэш, либо снимок, и это не настройка, а свойство: они альтернативы.

**Следствие 2 — чего эксперимент стоил.** Без него ошибка с путём политик вылезла бы не на стенде,
а у первого, кто выкатил бы образ в кластер: у него снимок восстановился бы «успешно» по логу warp
и упал в JVM. Это тот же класс, что wildcard из B-31, и снова его поймал прогон настоящим путём,
а не рассуждение.

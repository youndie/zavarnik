...
**Следствие.** Для konekt CRaC — смена базового образа на `azul/zulu-openjdk:25-jre-crac`
(та же 25.0.4.1), политика для слушающего сокета и ответ на вопрос про пул. Всё остальное —
тренировка в контейнере на сети стенда, слой поверх образа, проверка restore — уже есть в
`scripts/aot-image.sh` по форме.
...
Порядок: [B-32](../backlog/B-32-konekt-hikari-exposed-restore.md) (ворота) →
[B-33](../backlog/B-33-random-after-restore.md) → [B-34](../backlog/B-34-crac-plugin-form.md) →
[B-35](../backlog/B-35-crac-cpu-features.md). Эксперименты первой и второй фаз остаются

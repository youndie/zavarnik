...
- [B-42](docs/backlog/B-42-warmup-gate-on-printcompilation.md) `[x]` - A warm-up gate on an instrument that reports: jdk.Compilation switched on, inlining evidence taken elsewhere
- [B-43](docs/backlog/B-43-static-scan-across-owners.md) `[x]` - Re-run the scan and the join once the stand has a database
- [B-44](docs/backlog/B-44-calibration-controls-and-the-known-order-pair.md) `[x]` - Five calibration controls plus a pair whose order the code already decides
...
- [B-47](docs/backlog/B-47-rq2-rq3-continuation-machinery.md) `[x]` - RQ2 and RQ3: the one call site every suspend body shares, and the fast path that never suspends
- [B-48](docs/backlog/B-48-rq1-rq5-sizes-and-codegen-patterns.md) `[x]` - RQ1 and RQ5: the size threshold with the one dial that exists, and the codegen patterns counted across owners
- [B-49](docs/backlog/B-49-rq7-steady-state-and-the-compilers-own-cpu.md) `[x]` - RQ7: is steady state stable — and what the compiler itself costs under a container limit
